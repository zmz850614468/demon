...
export abstract class AbstractDao<T, K> {
  ...
  public static allTableIsCreateEnd() {
    let entityArr = GlobalContext.getContext().getValue("entityClsRelationshipArr");
    for (let entityClsRelationshipArrKey in entityArr) {

      // 处理ToOne
      let toOneRelations = GlobalContext.getContext().getValue("entityClsRelationshipArr")[entityClsRelationshipArrKey]['toOneRelations']
      ...
      // 处理ToMany
      let toManyRelations = GlobalContext.getContext().getValue("entityClsRelationshipArr")[entityClsRelationshipArrKey]['toManyRelations']
      if (toManyRelations !== undefined && toManyRelations.length > 0) {
        toManyRelations.forEach(item => {
          ...
          let incomingToManyRelations = GlobalContext.getContext().getValue("entityClsRelationshipArr")[targetClsName].incomingToManyRelations
          if (incomingToManyRelations !== undefined && incomingToManyRelations.length > 0) {
            GlobalContext.getContext().getValue("entityClsRelationshipArr")[targetClsName].incomingToManyRelations = incomingToManyRelations.concat([item])
          } else {
            GlobalContext.getContext().getValue("entityClsRelationshipArr")[targetClsName].incomingToManyRelations = [item]
          }
        })
      }

      // 处理 joinEntity
      let joinEntityTempList = GlobalContext.getContext().getValue("entityClsRelationshipArr")[entityClsRelationshipArrKey]['joinEntityTempList']
      if (joinEntityTempList !== undefined && joinEntityTempList.length > 0) {
        ...
        joinEntityTempList.forEach(item => {
          ...
          // 封装joinEntity的值
          let clsProperty = GlobalContext.getContext().getValue("entityClsRelationshipArr")[entityName]
          ...
          let incomingToManyRelations = GlobalContext.getContext().getValue("entityClsRelationshipArr")[targetClsName].incomingToManyRelations
          if (incomingToManyRelations !== undefined && incomingToManyRelations.length > 0) {
            GlobalContext.getContext().getValue("entityClsRelationshipArr")[targetClsName].incomingToManyRelations = incomingToManyRelations.concat([toManyWithJoinEntity])
          } else {
            GlobalContext.getContext().getValue("entityClsRelationshipArr")[targetClsName].incomingToManyRelations = [toManyWithJoinEntity]
          }
        ...
        let toManyRelations = GlobalContext.getContext().getValue("entityClsRelationshipArr")[entityClsRelationshipArrKey].toManyRelations
        if (toManyRelations !== undefined && toManyRelations.length > 0) {
          GlobalContext.getContext().getValue("entityClsRelationshipArr")[entityClsRelationshipArrKey].toManyRelations = toManyRelations.concat(joinEntity)
        } else {
          GlobalContext.getContext().getValue("entityClsRelationshipArr")[entityClsRelationshipArrKey].toManyRelations = joinEntity
        }

      }
      let entity = GlobalContext.getContext().getValue("entityClsRelationshipArr")[entityClsRelationshipArrKey]
      GlobalContext.getContext().getValue("entityClsRelationship")[entityClsRelationshipArrKey] = entity;
    }

    let res = GlobalContext.getContext().getValue("entityClsRelationshipArr")
  }
  ...
  private static getFkProperties(className, itemName): Property {
    let obj = GlobalContext.getContext().getValue("entityCls")[className]
    if (obj) {
  ...
  public static generatorPropertyArr(entityCls: any): Property[] {
    ...
    columns.forEach((item, idx) => {
      ...
      if (columnName) {
        ...
        if (index_unique != undefined) {
          arr[index].setIndex({ unique: index_unique, name: index_name });
        }
    ...
    GlobalContext.getContext().getValue("entityCls")[entityCls.name] = properties;
