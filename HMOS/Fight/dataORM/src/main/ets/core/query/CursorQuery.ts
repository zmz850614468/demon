...
export class CursorQuery<T> extends AbstractQueryWithLimit<T> {
  ...
  public async query(): Promise<any> {
    return await typeof this.predicates == 'string' ? this.dao.getDatabase().rawQueries(<string> this.predicates, this.parameters) : this.dao.getDatabase().rawQuery(<dataRdb.RdbPredicates> this.predicates, this.parameters);
  }
