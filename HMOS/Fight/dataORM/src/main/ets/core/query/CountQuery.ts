...
export class CountQuery<T> extends AbstractQuery<T> {
  ...
  public async count(): Promise<number> {
    let cursor: any = await typeof this.predicates == 'string' ? this.dao.getDatabase().rawQueries(<string> this.predicates) : this.dao.getDatabase().rawQuery(<dataRdb.RdbPredicates> this.predicates);
    try {
